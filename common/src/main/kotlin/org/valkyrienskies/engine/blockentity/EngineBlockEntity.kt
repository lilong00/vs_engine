...
class EngineBlockEntity(pos: BlockPos, state: BlockState) :
    ...
    private var maxEffectiveFuel = 100f - EngineConfig.SERVER.engineHeatGain
    private var lastFuelValue = 1600; // coal: 1600

    override fun createMenu(containerId: Int, inventory: Inventory): AbstractContainerMenu =
        EngineScreenMenu(containerId, inventory, this)

    override fun getDefaultName(): Component = TranslatableComponent("gui.vs_engine.engine")
        ...
        if (!this.level!!.isClientSide) {
            ...
            if (!level!!.hasNeighborSignal(blockPos)) {
                if (fuelLeft > 0) {
                    ...
                    if (EngineConfig.SERVER.engineFuelSaving) {
                        if (heat <= maxEffectiveFuel) {
                            heat += scaleEngineHeating(EngineConfig.SERVER.engineHeatGain)
                            fuelLeft--
                        }
                    } else {
                        fuelLeft--
                    ...
                    if (!fuel.isEmpty && lastFuelValue <= EngineConfig.SERVER.engineMinCapacity - fuelLeft) {
                        consumeFuel()
                    }
            ...
            if (heat > 0) {
               ...
               if (ship != null && engineShipControl != null) {
                   ...
                   engineShipControl!!.power += lerp(
                       EngineConfig.SERVER.minEnginePower,
                       EngineConfig.SERVER.enginePower,
                       effectiveHeat,
                   )

                   heat -= engineShipControl!!.consumed;
               }

                heat = max(heat - scaleEngineCooling(EngineConfig.SERVER.engineHeatLoss),0f)
            }
        }
    }
     ...
     */
    private fun getScaledFuel(): Int = ((FurnaceBlockEntity.getFuel()[fuel.item] ?: 0) * EngineConfig.SERVER.engineFuelMultiplier).toInt()
    ...
    private fun consumeFuel() {
        ...
        if (lastFuelValue > 0) {
            if (fuelLeft > 0 && lastFuelValue > EngineConfig.SERVER.engineMinCapacity - fuelLeft) {
                return
            }

            fuelLeft += lastFuelValue
            fuelTotal = max(lastFuelValue, EngineConfig.SERVER.engineMinCapacity)
     ...
     */
    private fun scaleEngineHeating(value: Float): Float = (100 * EngineConfig.SERVER.engineHeatChangeExponent -
            this.heat * EngineConfig.SERVER.engineHeatChangeExponent + 1f) * value
     ...
     * @return the scaled value.
     */
    private fun scaleEngineCooling(value: Float): Float = (this.heat * EngineConfig.SERVER.engineHeatChangeExponent + 1f) * value
